...
function wireTabs({ active, programId, teamId } = {}) {
  ...
  wireSwipeNav(targets, active);
}

// ---------------------------------------------------------------------
// Swipe left/right to move between tabs.
//
// Wired from inside wireTabs because that is where the destinations are
// already resolved, and because the pages that do NOT call it -- the
// index.html router and search.html -- are exactly the pages that have
// no tab bar and should not swipe.
//
// The order comes from the DOM rather than a constant here, so the
// gesture can never disagree with the bar the player is looking at.

// A real swipe, not a sloppy tap or a diagonal scroll: it has to travel
// far enough AND be decisively more horizontal than vertical, or every
// flick down the feed would fire one.
const SWIPE_MIN_X = 50;
const SWIPE_X_OVER_Y = 2;
// iOS reserves the screen edges for its own back/forward gesture in
// standalone mode. Starting a tab swipe there fights the OS and loses.
const SWIPE_EDGE_GUARD = 24;
const SLIDE_OUT_MS = 130;
const SLIDE_KEY = 'thirdcoast-nav-slide';

// Where a swipe would land, or null if there is nowhere to go. Separated
// from the gesture itself so the routing is testable without synthesising
// touches -- wireSwipeNav exposes a bound copy as window.swipeDestination
// for the UI suite.
//
// Deliberately NOT named swipeDestination: in a classic script a
// top-level function declaration IS a property of window, so assigning
// window.swipeDestination would overwrite this declaration and the
// wrapper would end up calling itself. That is a stack overflow, not a
// shadowing warning.
function resolveSwipeDestination(targets, active, step) {
  const order = [...document.querySelectorAll('.tabbar .tab')].map((el) => el.dataset.tab);
  let i = order.indexOf(active);
  if (i === -1) return null;
  // Walk past tabs with no destination (Ranks/Schedule with no saved
  // program) rather than stopping dead on one. No wrap-around: running
  // off the end of the bar should feel like the end of the bar.
  for (i += step; i >= 0 && i < order.length; i += step) {
    const href = targets[order[i]];
    if (href) return href;
  }
  return null;
}

function wireSwipeNav(targets, active) {
  const screenEl = document.querySelector('.screen');
  if (!screenEl || screenEl.dataset.swipeWired) return;
  screenEl.dataset.swipeWired = '1';
  window.swipeDestination = (step) => resolveSwipeDestination(targets, active, step);

  let sx = 0;
  let sy = 0;
  let tracking = false;

  // Both listeners are passive: this never calls preventDefault, so
  // vertical scrolling inside .body is completely untouched by it.
  screenEl.addEventListener('touchstart', (e) => {
    if (e.touches.length !== 1) { tracking = false; return; }
    const t = e.touches[0];
    if (t.clientX < SWIPE_EDGE_GUARD || t.clientX > window.innerWidth - SWIPE_EDGE_GUARD) {
      tracking = false;
      return;
    }
    sx = t.clientX;
...
// These are nine separate documents, not a single-page app, so a swipe
// cannot track the finger across a transition. What it can do is split
// the motion in two: this page slides off, and the next one slides in
// from the opposite side as it loads. sessionStorage carries the
// direction across the navigation -- it survives a same-tab load and is
// gone by the next session, which is exactly the lifetime wanted.
function navigateBySwipe(step) {
  const href = window.swipeDestination(step);
  if (!href) return;
  const screenEl = document.querySelector('.screen');
  const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduced || !screenEl) { location.href = href; return; }
  try {
    sessionStorage.setItem(SLIDE_KEY, step > 0 ? 'from-right' : 'from-left');
  } catch { /* private mode: the slide-in is a nicety, not a requirement */ }
  screenEl.classList.add(step > 0 ? 'nav-out-left' : 'nav-out-right');
  setTimeout(() => { location.href = href; }, SLIDE_OUT_MS);
}

// The other half of that motion, run on arrival.
function playSlideIn() {
  let dir = null;
  try {
    dir = sessionStorage.getItem(SLIDE_KEY);
    if (dir) sessionStorage.removeItem(SLIDE_KEY);
  } catch { return; }
  if (!dir) return;
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const screenEl = document.querySelector('.screen');
  if (!screenEl) return;
  const cls = dir === 'from-right' ? 'nav-in-right' : 'nav-in-left';
  screenEl.classList.add(cls);
  screenEl.addEventListener('animationend', () => screenEl.classList.remove(cls), { once: true });
}
document.addEventListener('DOMContentLoaded', playSlideIn);
